STACK_NAME=$1
VPC_NAME=${STACK_NAME}-demo-vpc
SUBNET1_NAME=${STACK_NAME}-demo-subnet1
SUBNET2_NAME=${STACK_NAME}-demo-subnet2
SUBNET3_NAME=${STACK_NAME}-demo-subnet3
IG_NAME=${STACK_NAME}-demo-InternetGateway
ROUTE_TABLE_NAME=${STACK_NAME}-demo-public-route-table
...
then 
	...
	export SUBNET1_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --availability-zone "eu-central-1a" --cidr-block 10.0.1.0/24 --query 'Subnet.SubnetId' --output text)
	if [ $? -eq 0 ]
	then
	...
	export SUBNET2_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --availability-zone "eu-central-1a" --cidr-block 10.0.2.0/24 --query 'Subnet.SubnetId' --output text)
	if [ $? -eq 0 ]
	then
	...
	export SUBNET3_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --availability-zone "eu-central-1a" --cidr-block 10.0.3.0/24 --query 'Subnet.SubnetId' --output text)
	if [ $? -eq 0 ]
	then
